...
type
  TfrCaixa = class(TUniFrame)
    ...
    cdVendasVALORTOTAL: TFloatField;
    cdVendasagregado: TAggregateField;
    lbvalotT: TUniLabel;
...
TOTAL : Real;
begin
  TOTAL:= StrToFloat( lbvalotT.Caption );

   with cdVendas do
   begin
   cdVendas.DisableControls;
   cdVendas.First;
    while not cdVendas.Eof do
    begin
     TOTAL := (cdVendas.fieldByName('TOTAL').AsInteger);
     cdVendas.next;
    end;
    cdVendas.EnableControls;
    lbvalotT.caption := FormatFloat('#,##0.00', TOTAL ) ; // Format('%00.0n', [vidautil]);
   end;
   cdVendas.Delete;
end;
...
begin
  ...
  begin
   ...
   cdVendas.First;
    while not cdVendas.Eof do
    begin
     SOMAR := (SOMAR+cdVendas.fieldByName('TOTAL').AsInteger);
     cdVendas.next;
    end;
    cdVendas.EnableControls;
    lbvalotT.caption := FormatFloat('#,##0.00', SOMAR ) ; // Format('%00.0n', [vidautil]);
   end;
